...
// Both the icon and the "DROPIX" wordmark are white silhouettes on
// transparent PNGs (icon-mask.png / dropix-text-mask.png) — CSS mask-image
...
export function LogoPreview({ variant, colors, size = 72 }: { variant: 'badge' | 'wordmark'; colors: readonly string[]; size?: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      {variant === 'wordmark' && (
        <MaskedGradient mask="/logos/dropix-text-mask.png" width={size * 1.05} height={size * 1.05 * TEXT_ASPECT} gradient={gradient} />
      )}
    </div>
  );
}
